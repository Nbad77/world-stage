    ...
    if counter_offer is None and dialogue_text and _detect_unstructured_payment(dialogue_text):
        try:
            ...
            _fix_g_prompt = (
                ...
                f"Negotiation cap: ${negotiation_cap}B maximum per deal.\n\n"
                f"{npc_rules}\n\n"
                f"EXTRACTION RULES — READ CAREFULLY:\n"
                f"Only extract payments the NPC explicitly committed to in THIS message. "
                f"A payment is committed if the NPC used language like: "
                f"'I will pay', 'you will receive', 'I am offering', "
                f"'the deal is', 'here is what I can do', 'I can give you'.\n\n"
                f"Do NOT extract payments that are:\n"
                f"- Conditional on future behavior ('if you follow through...', 'should you...')\n"
                f"- Framed as future possibilities ('we could revisit', 'potential', 'maybe', "
                f"'perhaps', 'down the line', 'that is when real money moves')\n"
                f"- Relationship-contingent ('we will see how things develop', 'depends on...')\n"
                f"- Already declined earlier in the same message "
                f"('I cannot commit to X... but here is Y' — only extract Y)\n\n"
                f"If no committed payment exists in this message, return "
                f"{{\"counter_offer\": null}}.\n\n"
                f"If a committed payment IS present: Convert euros to dollars at 1:1. "
                f"Convert millions to billions (e.g. €900M = $0.9B). "
                f"If the dialogue mentions split payments or tranches, use installments. "
                f"Return a valid counter_offer JSON."
            )
