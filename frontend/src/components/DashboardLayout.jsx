...
export default function DashboardLayout({ gs, children, onShadowCabinet, negotiatingNpc, onHistorian, historianLoading, onContact, onContactRequest, contactLoading, contactResults, contactsDisabled, activeTab, onTabChange, domesticContent, onBackchannel, backchannelDisabled, onBiography, onGetIntel, intelLoading, intelResults, dialogue }) {
  ...
  return (
    <div className={`dashboard-shell ${mode === 'event' ? 'mode-event' : 'mode-ambient'}`}>
      ...
      <aside className="dashboard-right hidden lg:block">
        <RightSidebar gs={gs} onContact={onContact} onContactRequest={onContactRequest} contactLoading={contactLoading} contactResults={contactResults} negotiatingNpc={negotiatingNpc} contactsDisabled={contactsDisabled} onBackchannel={onBackchannel} backchannelDisabled={backchannelDisabled} onGetIntel={onGetIntel} intelLoading={intelLoading} intelResults={intelResults} dialogue={dialogue} />
      </aside>
